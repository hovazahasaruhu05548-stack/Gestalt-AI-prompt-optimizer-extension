...
(function() {
  ...
  const PLATFORM_SELECTORS = {
    ...
    doubao: {
      ...
      // 修复：拆分带空格的无效字符串，加入 volcengine 域
      urlPatterns: ['doubao.com', 'www.doubao.com', 'ark.cn', 'volc.cn', 'volcengine.com']
    },
    // Kimi
    kimi: {
      name: 'Kimi',
  ...
  function createFloatingButton() {
    ...
    floatingButton.innerHTML = `
      <div class="gestalt-fab-tooltip">单击优化 | 双击面板 | 拖拽移动</div>
      <div class="gestalt-fab-content">
    ...
    try {
      ...
      if (response?.success && response?.optimizedText) {
        ...
        if (verifyText.includes(optimizedText.substring(0, 20))) {
          ...
          // 修复：放弃使用只读的 ClipboardEvent 触发剪贴板赋值，改用原生的 execCommand 进行可靠粘贴
          inputElement.focus()
  ...
  function makeDraggable(element) {
    ...
    document.addEventListener('mousemove', (e) => {
      ...
      if (!hasMoved && distance > 5) {
        ...
        fabContent.style.transform = 'scale(1.05)'
      ...
      if (isDragging) {
        ...
        if (elementRect.left < 0) element.style.left = '0px'
        if (elementRect.top < 0) element.style.top = '0px'
  ...
  function getConversationHistory() {
    console.log('[Gestalt] 开始获取对话历史，当前平台:', currentPlatform?.key)

    if (!currentPlatform) {
      console.log('[Gestalt] 未检测到平台，返回空历史')
      return []
    }
    ...
    if (!selectors) {
      console.log('[Gestalt] 未找到平台选择器配置')
      return []
    }
    ...
    try {
      ...
      console.log('[Gestalt] 用户消息选择器:', userSelectors)
      console.log('[Gestalt] AI消息选择器:', assistantSelectors)

      // 获取用户消息
      for (const selector of userSelectors) {
        const elements = document.querySelectorAll(selector)
        console.log(`[Gestalt] 选择器 "${selector}" 找到 ${elements.length} 个元素`)
      ...
      for (const selector of assistantSelectors) {
        ...
        elements.forEach(el => {
          const text = el.textContent?.trim()
          // 放宽长度限制：2-100000字符
          if (text && text.length > 2 && text.length < 100000) {
            // 避免重复
      ...
      console.log('[Gestalt] 初步获取到消息数:', messages.length)

      // 按DOM顺序排序
      const allElements = document.querySelectorAll(selectors.conversation.join(','))
      console.log('[Gestalt] 总共找到对话元素:', allElements.length)
      ...
      const finalMessages = sortedMessages.slice(-20) // 最多返回最近20条
      console.log('[Gestalt] 最终返回对话历史:', finalMessages.length, '条')
      console.log('[Gestalt] 对话历史内容预览:', finalMessages.map(m => ({ role: m.role, length: m.content.length })))

      return finalMessages

    } catch (e) {
      console.error('[Gestalt] 获取对话历史失败:', e)
      return []
    }
  }
