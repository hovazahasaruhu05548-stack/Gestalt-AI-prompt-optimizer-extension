...
const BUILT_IN_TEMPLATES = {
  text: [
    { id: 't1', title: '💼 产品经理', prompt: '帮我设计一个专业的[产品经理]角色提示词，要求包含职责、能力要求、工作流程', tags: ['角色', '产品'] },
    { id: 't2', title: '🔒 代码安全', prompt: '帮我分析这段代码的安全漏洞，并提供修复建议', tags: ['代码', '安全'] },
    { id: 't3', title: '⚖️ 法律顾问', prompt: '扮演专业的法律顾问，帮我审查这份合同的风险点', tags: ['法律', '合同'] },
    { id: 't4', title: '📝 文案写作', prompt: '帮我写一段吸引人的产品介绍文案，突出核心卖点', tags: ['营销', '文案'] },
    { id: 't5', title: '🎓 论文指导', prompt: '帮我优化这篇学术论文的标题和摘要，使其更具吸引力', tags: ['学术', '论文'] },
    { id: 't6', title: '🌐 翻译专家', prompt: '将以下中文内容翻译成专业的英文，保持原文风格', tags: ['翻译', '语言'] },
    { id: 't7', title: '💡 头脑风暴', prompt: '帮我针对[主题]进行头脑风暴，提出10个创新想法', tags: ['创意', '头脑风暴'] },
    { id: 't8', title: '📊 数据分析', prompt: '分析这份数据，提取关键洞察并给出建议', tags: ['数据', '分析'] }
  ],
  image: [
    { id: 'i1', title: '🐱 可爱猫咪', prompt: '一只可爱的猫咪在花园里玩耍，阳光下毛发闪闪发亮', tags: ['动物', '可爱'] },
    { id: 'i2', title: '🌃 赛博朋克', prompt: '未来赛博朋克城市夜景，霓虹灯闪烁，雨天倒影', tags: ['城市', '科幻'] },
    { id: 'i3', title: '🏔️ 中国山水', prompt: '中国山水画风格，远山云雾缭绕，意境悠远', tags: ['风景', '艺术'] },
    { id: 'i4', title: '👩 人物肖像', prompt: '精致的人物肖像，柔和的光线，专业的摄影风格', tags: ['人物', '肖像'] },
    { id: 'i5', title: '🍔 美食摄影', prompt: '美味的食物摄影，高角度拍摄，新鲜食材特写', tags: ['美食', '摄影'] },
    { id: 'i6', title: '🏠 室内设计', prompt: '现代简约风格的室内设计，温暖的灯光，温馨舒适', tags: ['设计', '室内'] }
  ],
  video: [
    { id: 'v1', title: '☕ 咖啡教程', prompt: '制作一个咖啡冲泡过程的教程视频，从豆子到杯中', tags: ['教程', '美食'] },
    { id: 'v2', title: '📱 产品展示', prompt: '科技产品展示视频，流畅转场，现代感背景音乐', tags: ['产品', '商业'] },
    { id: 'v3', title: '🌆 延时摄影', prompt: '城市延时摄影，从白天到夜晚，车流如光', tags: ['风景', '延时'] },
    { id: 'v4', title: '🎮 游戏预告', prompt: '游戏预告片风格，快节奏剪辑，炫酷特效', tags: ['游戏', '预告'] }
  ],
  audio: [
    { id: 'a1', title: '☕ 咖啡厅音乐', prompt: '制作一首轻松的咖啡厅背景音乐，柔和的钢琴和吉他', tags: ['背景音乐', '放松'] },
    { id: 'a2', title: '🎮 游戏音乐', prompt: '赛博朋克风格的电子音乐，适合游戏场景', tags: ['电子', '游戏'] },
    { id: 'a3', title: '🎬 电影配乐', prompt: '电影预告片风格的史诗级管弦乐，气势磅礴', tags: ['史诗', '电影'] },
    { id: 'a4', title: '🌸 夏日青春', prompt: '创作一首关于夏天和青春的流行歌曲，清新明快', tags: ['流行', '青春'] }
  ]
}
...
let usageStats = {
  ...
  modeUsage: { text: 0, image: 0, video: 0, audio: 0 },
  favoriteMode: 'text'
}
...
const CONFIG = {
  ...
  storageKeys: {
    ...
    audio: 'gestalt_audio_state',
    prompts: 'gestaltPrompts',
    history: 'gestaltHistory',
    config: 'gestaltConfig',
    stats: 'gestaltUsageStats'  // 新增统计存储
  }
}
...
async function loadUsageStats() {
  try {
    const stored = await chrome.storage.local.get(CONFIG.storageKeys.stats)
    if (stored[CONFIG.storageKeys.stats]) {
      usageStats = { ...usageStats, ...stored[CONFIG.storageStats] }

      // 检查是否是同一天
      const today = new Date().toDateString()
      if (usageStats.lastDate !== today) {
        // 新的一天，重置今日统计
        usageStats.todayOptimizations = 0
        usageStats.lastDate = today
        await saveUsageStats()
      }
    }
  } catch (e) {
    console.log('[Gestalt] 加载统计失败:', e)
  }
}
...
async function saveUsageStats() {
  try {
    await chrome.storage.local.set({
      [CONFIG.storageKeys.stats]: usageStats
    })
  } catch (e) {
    console.log('[Gestalt] 保存统计失败:', e)
  }
}

function updateUsageStats(mode) {
  usageStats.totalOptimizations++
  usageStats.todayOptimizations++
  usageStats.modeUsage[mode] = (usageStats.modeUsage[mode] || 0) + 1

  // 更新最常用模式
  const maxMode = Object.entries(usageStats.modeUsage)
    .sort((a, b) => b[1] - a[1])[0]
  usageStats.favoriteMode = maxMode[0]
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.ctrlKey && e.key >= '1' && e.key <= '4') {
      ...
      if (modes[index]) {
        ...
        showToast(`已切换到 ${getModeName(modes[index])} 模式`, 'info')
      }
    }
...
function clearCurrentMode() {
  modeStates[currentMode].messages = []
  modeStates[currentMode].extractedPrompt = ''
  modeStates[currentMode].lastInput = ''

  // 隐藏输出
  const outputContainer = document.getElementById(`${currentMode}Output`)
  if (outputContainer) {
...
function showWelcomeIfFirstTime() {
  const hasUsed = localStorage.getItem('gestalt_has_used')

  if (!hasUsed) {
    // 首次使用，显示欢迎
    setTimeout(() => {
      showToast('🎉 欢迎使用 Gestalt！按 Ctrl+1-4 切换模式', 'info')
    }, 1000)

    localStorage.setItem('gestalt_has_used', 'true')
  }
}
...
const DAILY_TIPS = [
  '💡 提示：按 Ctrl+1-4 快速切换模式',
  '💡 提示：Ctrl+Enter 快速发送消息',
  '💡 提示：点击消息上的 ✏️ 可以编辑重新发送',
  '💡 提示：在提示词库中保存常用提示词',
  '💡 提示：历史记录可以复用之前的对话',
  '💡 提示：点击消息可以复制内容',
  '💡 提示：优化后的提示词可以直接插入到AI对话中',
  '💡 提示：使用快捷模板快速开始'
]
...
function showDailyTip() {
  ...
  if (Math.random() > 0.5) {
    setTimeout(() => {
      showToast(DAILY_TIPS[tipIndex], 'info', 4000)
...
function getTemplatesForMode(mode) {
  return BUILT_IN_TEMPLATES[mode] || []
}

function insertTemplate(prompt) {
  const input = document.getElementById(`${currentMode}Input`)
  if (input) {
    input.value = prompt
    input.focus()
...
function getWelcomeMessage(mode) {
  const shortcutsHint = `
    <div class="shortcuts-hint" style="margin-top: 16px; padding: 12px; background: linear-gradient(135deg, rgba(102,126,234,0.1), rgba(118,75,162,0.1)); border-radius: 10px; font-size: 12px; color: var(--text-secondary);">
      <div style="font-weight: 600; margin-bottom: 8px; color: var(--primary);">⌨️ 快捷键</div>
      <div style="display: flex; gap: 16px; flex-wrap: wrap;">
        <span><kbd style="background: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; border: 1px solid var(--border);">Ctrl+1-4</kbd> 切换模式</span>
        <span><kbd style="background: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; border: 1px solid var(--border);">Ctrl+Enter</kbd> 发送</span>
        <span><kbd style="background: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; border: 1px solid var(--border);">Esc</kbd> 关闭弹窗</span>
      </div>
    </div>
  `
...
async function handleSend(mode) {
  ...
  try {
    ...
    // 显示输出（即使提取失败也显示原始内容）
    const promptToDisplay = result.prompt || result.explanation || ''

    if (promptToDisplay) {
      modeStates[mode].extractedPrompt = promptToDisplay
      ...
      console.log('[Gestalt] 更新输出区域:', {
        ...
        displayLength: promptToDisplay.length,
      ...
      if (outputEl) {
        if (outputEl.tagName === 'TEXTAREA') {
          outputEl.value = promptToDisplay
        } else {
          outputEl.textContent = promptToDisplay
        }
        console.log('[Gestalt] 输出内容已更新到元素:', outputEl.id, '长度:', promptToDisplay.length)
      } else {
...
async function compilePromptWithRAG(userInput, mode, includeContext = true) {
  ...
  if (includeContext) {
    try {
      ...
      if (tabId) {
        console.log('[Gestalt] 正在获取对话历史...')

        const response = await chrome.tabs.sendMessage(tabId, {
          type: 'GET_CONVERSATION_HISTORY'
        }).catch(() => {
          console.log('[Gestalt] 获取对话历史超时或失败')
          return null
        })

        console.log('[Gestalt] 对话历史响应:', response)
        ...
        if (response?.history && response.history.length > 0) {
          ...
          contextInfo = `\n\n📚 **已启用记忆功能** - 读取到最近 ${contextMessages.length} 条对话记录`
          console.log('[Gestalt] 成功获取对话历史:', contextMessages.length, '条')
        } else {
          contextInfo = `\n\n⚠️ **记忆功能已启用但未读取到对话历史** - 可能是平台不支持或需要刷新页面`
          console.log('[Gestalt] 未获取到对话历史')
        }
      }
    } catch (e) {
      contextInfo = `\n\n❌ **获取对话历史失败** - ${e.message}`
  ...
  if (contextMessages.length > 0) {
    messages.push({
      role: 'system',
      content: `\n**对话历史参考（最近${contextMessages.length}条）：**\n使用这些上下文来更好地理解用户需求和保持对话连贯性。`
    })
    messages.push(...contextMessages)
  }

  // 将上下文信息添加到系统提示词中（让用户知道）
...
function extractPrompt(text) {
  ...
  console.log('[Gestalt] extractPrompt 输入:', text.substring(0, 100) + '...')

  // 尝试多种格式提取
  // 1. Markdown 代码块 (优先级最高)
  let match = text.match(/```(?:markdown|prompt)?\n?([\s\S]*?)\n?```/)
  if (match) {
    console.log('[Gestalt] 使用 Markdown 代码块提取')
    return match[1].trim()
  }

  // 2. 对于图片/视频模式，尝试提取英文段落（通常是提示词）
  // 检测是否包含大量英文内容（图片提示词通常是英文）
  const englishMatch = text.match(/([a-zA-Z\s\.,;:!?'"()\-]{50,})/g)
  if (englishMatch && englishMatch.length > 0) {
    // 取最长的英文段落
    const longestEnglish = englishMatch.sort((a, b) => b.length - a.length)[0]
    if (longestEnglish.length > 30) {
      console.log('[Gestalt] 使用英文段落提取')
      return longestEnglish.trim()
    }
  }
  ...
  match = text.match(/【[^】]*】[\s：:]*\n([\s\S]+?)(?=\n\n|$)/)
  if (match) {
    console.log('[Gestalt] 使用【】标记提取')
    return match[1].trim()
  }

  // 5. 如果包含"优化思路"等标记，尝试提取后面的内容
  const sections = text.split(/(?:优化思路|设计思路|说明)[：:]\s*/i)
  if (sections.length > 1) {
    // 取第一段之后的内容
    const promptContent = sections.slice(1).join('').trim()
    if (promptContent.length > 20) {
      console.log('[Gestalt] 使用分段提取')
      return promptContent
    }
  }

  // 6. 整个文本（最后fallback）
  console.log('[Gestalt] 使用整个文本')
  return text.trim()
}
...
function showToast(message, type = 'info', duration = 3000) {
  ...
  if (icons[type]) {
    toast.textContent = icons[type] + ' ' + toast.textContent
  }
